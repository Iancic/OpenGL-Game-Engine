...
void Segment::Update()
{
	...
	int boxSize = segmentRadius * 2;
	collider->Update(&transform, boxSize, boxSize);
}

void Segment::Render(Camera* camera)
{
	Circle::getInstance()->DrawCircle(transform.position.x, transform.position.y, segmentRadius, segmentShape, glm::vec4(255, 255, 255, 255));
}

void Segment::RenderDebug()
{
	collider->Render();
}
